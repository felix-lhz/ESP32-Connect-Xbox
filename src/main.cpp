...
// 需要在此替换成自己的手柄蓝牙MAC地址
XboxSeriesXControllerESP32_asukiaaa::Core xboxController("ac:8e:bd:14:55:d1");
// 创建麦克纳姆轮驱动器对象
...
void demoVibration_2() {
    ...
    repo.v.select.left = true;
    repo.v.select.right = true;
    repo.v.power.left = (uint8_t)((float)xboxController.xboxNotif.trigLT /
                                  (float)TrigMax * 100);
    repo.v.power.right = (uint8_t)((float)xboxController.xboxNotif.trigRT /
                                   (float)TrigMax * 100);
...
void loop() {
    ...
    if (xboxController.isConnected()) {
        if (xboxController.isWaitingForFirstNotification()) {
            ...
            //   demoVibration();
            //   demoVibration_2();
            double LV = 0.5 - xboxController.xboxNotif.joyLVert / 65535.0;
            double RH = xboxController.xboxNotif.joyRHori / 65535.0 - 0.5;
            if(abs(LV) < 0.1) LV = 0;
            if(abs(RH) < 0.1) RH = 0;
            double Vspeed = LV * speed * 2;
            double Hspeed = RH * speed * 2;
            Serial.println(Vspeed);
            Serial.println(Hspeed);
            speeds[0] = Vspeed + Hspeed;
            speeds[1] = Vspeed - Hspeed;
            speeds[2] = Vspeed + Hspeed;
            speeds[3] = Vspeed - Hspeed;
            mecanum.driveAllMotor(speeds[0], speeds[1], speeds[2], speeds[3]);
        }
    } else {
